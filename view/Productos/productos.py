...
# Importamos el Modelo y Controladores
...
# --- IMPORTAR EL HEADER DE TU EQUIPO ---
try:
...
class EstiloBase(Frame):
    ...
    def __init__(self, master, controlador, titulo):
        ...
        # Instanciamos el header externo (código de tu amigo)
        try:
            self.encabezado = header(self, controlador)
            self.encabezado.pack(side="top", fill="x")
            # Usamos la propiedad .titulo del header para cambiar el texto
